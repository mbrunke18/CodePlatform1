...
/**
 * The Proactive AI Radar - Core of Readiness OS Strategic Intelligence
 * 
...
export class ProactiveAIRadar {
  ...
  private async evaluateDataAgainstTrigger(
    ...
    try {
      const prompt = `
You are the AI component of M's Proactive Strategic Radar. Analyze the following data point against the specified trigger pattern.
...
`;
  ...
  async generateSyntheticScenarios(organizationId: string, query: string): Promise<any[]> {
    try {
      const prompt = `
You are the Synthetic Futures Engine of M, tasked with generating novel strategic scenarios beyond historical templates.
...
`;
  ...
  async validateExecutiveIntuition(intuitionRecord: {
    ...
    try {
      const prompt = `
You are M's Intuition Validation AI. An executive has recorded a strategic hunch that needs validation.
