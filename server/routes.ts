...
// Escapes a value for safe interpolation into HTML email templates built via
// template literals (no JSX auto-escaping is available server-side). Coerces
// non-strings so callers can pass numbers/undefined without crashing.
...
// Only allow a shareUrl into an outbound email if it actually points back at
// this app's own origin — prevents the field being used to smuggle an
// arbitrary link (e.g. javascript:, or a phishing domain) into a trusted
// transactional email.
function sanitizeShareUrl(shareUrl: unknown, req: any): string | null {
...
export async function registerRoutes(app: Express, existingServer?: Server): Promise<Server> {
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  // This function wires every API route. Use Ctrl+F on the section markers below.
  //
  //  §A  Auth setup, rate limiters, middleware (this block)             ~L784
  //  §B  Imported module registrations (activation, demo, onboarding…)  ~L841
  //  §C  Auth Entry Points — Magic Link · Founding Partner · Trial       ~L865
  //      → registerMagicLinkRoutes(app)  [server/routes/magic-link-routes.ts]
  //  §D  Strategic Enhancement Routes — War Room, Briefings, Alerts      ~L1076
  //      Executive analytics, Board-Ready Reporting, Preparedness Score
  //  §E  Playbook Routes — Pre-Armed Signals, Trigger Groups, Library    ~L2927
  //  §F  AI Enterprise Intelligence — Proactive Radar, Triggers          ~L4009
  //  §G  Activation Orchestration — Drills, ROI, Signal Ontology         ~L7446
  //  §H  Role Availability, Activation Outcomes, Admin Health            ~L8486
  //      Maturity Score, Playbook Performance Fingerprint
  //  §I  Signal Intelligence Routes                                       ~L8766
  //      → registerSignalIntelligenceRoutes(app)  [server/routes/signal-intelligence-routes.ts]
  //  §J  WOW Feature Routes — Compound Threats, Investor Gate            ~L8769
  //      Task Acknowledgments, Execution History, Crisis Comms
  //  §K  Situation Intents, Executive Readiness Score                     ~L10023
  //  §L  Board-Ready Activation Report                                    ~L10157
  //  §M  Custom Protocols (Protocol Builder), Certification               ~L10472
  // ══════════════════════════════════════════════════════════════════════════════
  ...
  // Strict limiter for public lead-capture / transactional email endpoints —
  // these send outbound email via Resend and must not be usable as a spam
  // relay or to exhaust the sending quota.
  ...
  // ── Auth Entry Points: Magic Link · Founding Partner · Trial Access ──────────
...
Return ONLY a JSON object with this exact structure (no markdown, no explanation):
{
  ...
  // Task Templates — org-scoped task definitions persisted as action items
  const toTaskTemplate = (r: any) => ({
    id: r.id,
    title: r.title,
    description: r.description,
    assignedRole: r.assignedTo,
    priority: r.priority,
    status: (r.tags as any)?.status || 'pending',
    estimatedMinutes: (r.tags as any)?.estimatedMinutes || 15,
    approvalRequired: (r.tags as any)?.approvalRequired || 'none',
    deliverables: r.outcome || '',
    dependsOn: (r.dependencies as any) || [],
    templateId: (r.tags as any)?.templateId || null,
  });

  app.get('/api/task-templates', async (req: any, res) => {
    try {
      const userId = getUserId(req);
      ...
      const rows = await db
        .select()
        .from(actionItems)
        .where(and(eq(actionItems.organizationId, orgId), sql`${actionItems.tags}->>'isTaskTemplate' = 'true'`))
  ...
  app.post('/api/task-templates', async (req: any, res) => {
    try {
      const userId = getUserId(req);
      if (!userId) return res.status(401).json({ message: 'Unauthorized' });
      const orgId = await getOrgIdForUser(userId);
  ...
  app.put('/api/task-templates/:id', async (req: any, res) => {
    try {
      const userId = getUserId(req);
      if (!userId) return res.status(401).json({ message: 'Unauthorized' });
      const orgId = await getOrgIdForUser(userId);
      ...
      const [row] = await db.update(actionItems).set({
        ...
        tags: { isTaskTemplate: true, status: status || 'pending', estimatedMinutes: estimatedMinutes || 15, approvalRequired: approvalRequired || 'none', templateId: templateId || null },
        updatedAt: new Date(),
      }).where(and(eq(actionItems.id, id), eq(actionItems.organizationId, orgId))).returning();
  ...
  app.delete('/api/task-templates/:id', async (req: any, res) => {
    try {
      const userId = getUserId(req);
      if (!userId) return res.status(401).json({ message: 'Unauthorized' });
      const orgId = await getOrgIdForUser(userId);
      if (!orgId) return res.status(403).json({ message: 'No organization found' });
      const { id } = req.params;
      await db.delete(actionItems).where(and(eq(actionItems.id, id), eq(actionItems.organizationId, orgId)));
      res.json({ success: true });
    } catch (error) {
...
Generate realistic transformation metrics for a startup to Fortune 500 ${industry} company undertaking strategic digital transformation.`
  ...
  app.post('/api/stakeholder-contacts', async (req: any, res) => {
    try {
      ...
      const [contact] = await db.insert(stakeholderContactsTable).values({
        organizationId, role, name, email,
        phone: phone || null,
        preferredChannel: preferredChannel || 'email',
        slackUserId, slackChannel,
  ...
  // PUT /api/stakeholder-contacts/:id — update a contact
  app.put('/api/stakeholder-contacts/:id', async (req: any, res) => {
    try {
      const { id } = req.params;
      const { eq } = await import('drizzle-orm');
      const { role, name, email, phone, preferredChannel, slackUserId, slackChannel, isActive, metadata } = req.body;
      const [contact] = await db.update(stakeholderContactsTable)
        .set({
          ...(role !== undefined && { role }),
          ...(name !== undefined && { name }),
        ...
        .where(eq(stakeholderContactsTable.id, parseInt(id)))
        .returning();
      res.json({ success: true, contact });
    } catch (err) {
      res.status(500).json({ success: false });
  ...
  // Recent activations — org-scoped, used by Dashboard (last 5)
  app.get('/api/playbook-activations/recent', async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const orgId = userId ? await getOrgIdForUser(userId) : null;
      const whereClause = orgId ? eq(playbookActivations.organizationId, orgId) : sql`1=1`;
      ...
      // Map fields to what the Dashboard component expects
  ...
  // Demo seeder — creates 4 realistic activations for a fresh org (idempotent)
  app.post('/api/demo/seed-activations', isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      ...
      // Idempotent — skip if org already has activations
      const [existing] = await db.select({ count: count() })
        .from(playbookActivations)
        .where(eq(playbookActivations.organizationId, orgId));
      if ((existing?.count ?? 0) > 0) return res.json({ seeded: false, reason: 'already-seeded' });

      // Get one playbook from each of up to 4 domains for scenario variety
      const domainRows = await db.selectDistinct({ domainId: playbookLibrary.domainId })
        .from(playbookLibrary)
        .where(eq(playbookLibrary.isActive, true))
        .limit(4);
      ...
      for (const { domainId } of domainRows) {
        const [pb] = await db.select({ id: playbookLibrary.id })
          .from(playbookLibrary)
          .where(and(eq(playbookLibrary.domainId, domainId), eq(playbookLibrary.isActive, true)))
          .limit(1);
      ...
      const pid = (i: number) => pbIds[i % pbIds.length];

      const scenarios: Array<{
        playbookId: string; activationReason: string; situationSummary: string;
        actualExecutionTime: number | null; targetMet: boolean | null; successRating: number | null;
        activatedAt: Date; completedAt: Date | null;
        tasks: Array<{ taskName: string; sequence: number; phase: string; ownerRole: string; status: string; estimatedMinutes: number }>;
      }> = [
        {
          playbookId: pid(0),
          ...
          actualExecutionTime: 9, targetMet: true, successRating: 94,
          activatedAt: ago(18), completedAt: ago(18),
          tasks: [
        ...
        },
        {
          ...
          actualExecutionTime: 11, targetMet: true, successRating: 97,
          activatedAt: ago(9), completedAt: ago(9),
          tasks: [
        ...
        },
        {
          ...
          actualExecutionTime: 12, targetMet: true, successRating: 89,
          activatedAt: ago(3), completedAt: ago(3),
          tasks: [
        ...
        },
        {
          ...
          actualExecutionTime: null, targetMet: null, successRating: null,
          activatedAt: ago(0), completedAt: null,
          tasks: [
      ...
      for (const scenario of scenarios) {
        const { tasks, ...activationData } = scenario;
        const [activation] = await db.insert(playbookActivations).values({
          organizationId: orgId,
          activatedBy: userId,
          ...activationData,
        }).returning();
        if (activation?.id && tasks.length > 0) {
          await db.insert(activationTasks).values(tasks.map(t => ({ activationId: activation.id, ...t })));
        }
        seededCount++;
      }
...
Write the summary in third person past tense. Focus on velocity, team coordination, and lessons captured. Do not use bullet points.`;
  ...
  // ── Signal Intelligence Routes (monitoring · calibration · connectors · profiles) ─
  registerSignalIntelligenceRoutes(app);
...
Respond ONLY as JSON with this exact structure:
{
  ...
  app.post('/api/test-drive/email-summary', leadCaptureLimiter, async (req: any, res) => {
    try {
  ...
  app.post('/api/situation-scanner/lead', leadCaptureLimiter, async (req: any, res) => {
    try {
  ...
  app.post('/api/roi-calculator/email-report', leadCaptureLimiter, async (req: any, res) => {
    try {
              ...
              <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;max-width:600px;width:100%;">
                ...
                <tr><td style="padding:40px;">
                  ...
                  <div style="padding:20px 24px;background:#f0faf6;border-left:4px solid ${TEAL_R};margin-bottom:28px;">
                    <p style="font-size:14px;color:#374151;line-height:1.7;margin:0;">
                      3-year net value at ${fmtC(safePlatformCost)}/yr platform investment: <strong>${fmtC(safeThreeYearValue)}</strong>. That is the difference between mobilizing from scratch every time a strategic trigger fires, and executing from a pre-staged position in 12 minutes — a <strong>3,600× Execution Head Start</strong>.
                    </p>
                  </div>
                  ${safeShareUrl ? `
