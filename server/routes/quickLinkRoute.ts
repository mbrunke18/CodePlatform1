...
<body style="margin:0;padding:0;background:#f5f5f5;font-family:'Segoe UI',Arial,sans-serif;">
  ...
  // Anyone who clicks it gets their own session; no name/email required.
  // Optional: sendEmail + recipientEmail to deliver the group link to a specific address.
  app.post("/api/admin/generate-group-link", requirePlatformAdmin, async (req: any, res) => {
    try {
      const { linkDays = 7, sessionHours = 72, sendEmail = false, recipientEmail, recipientName } = req.body;
      ...
      if (sendEmail) {
        if (!recipientEmail) {
          emailError = "recipientEmail is required when sendEmail is true";
        } else {
          const apiKey = process.env.RESEND_API_KEY;
          if (!apiKey) {
            emailError = "RESEND_API_KEY not configured";
            console.warn("[GroupLink] Email skipped — RESEND_API_KEY not set");
          } else {
            try {
              const resend = new Resend(apiKey);
              const firstName = recipientName ? (recipientName.split(" ")[0] || recipientName) : "there";
              const NAVY = "#0A0F2E";
              const GOLD = "#C9A84C";
              const expiryLabel = linkDaysCapped === 1 ? "1 day" : `${linkDaysCapped} days`;
              const html = `<!DOCTYPE html>
<html><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#f5f5f5;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f5;padding:40px 0;">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;">
        <tr><td style="background:${NAVY};padding:28px 36px;">
          <div style="color:${GOLD};font-size:10px;font-weight:700;letter-spacing:3px;text-transform:uppercase;margin-bottom:6px;">VAUGHNMARTIN · READINESS OS</div>
          <div style="color:#ffffff;font-size:20px;font-weight:700;">Your Readiness OS Demo Access</div>
        </td></tr>
        <tr><td style="height:3px;background:${GOLD};"></td></tr>
        <tr><td style="padding:36px;">
          <p style="margin:0 0 8px;color:#111827;font-size:16px;font-weight:600;">Hi ${firstName},</p>
          <p style="margin:0 0 28px;color:#374151;font-size:15px;line-height:1.7;">
            You've been granted access to the Readiness OS platform. 
            Explore live trigger detection, 180 pre-staged Readiness Protocols, Mission Control, 
            and the complete IDEA Framework — your session starts the moment you click the link below.
          </p>
          <table cellpadding="0" cellspacing="0" style="margin:0 0 28px;">
            <tr>
              <td style="background:${NAVY};padding:14px 32px;">
          ...
          <p style="margin:0 0 6px;color:#6B7280;font-size:13px;line-height:1.6;">
            This link is shareable and active for <strong>${expiryLabel}</strong>. Each person who clicks it receives their own <strong>${sessionHoursCapped}-hour</strong> session.<br/>
            If you have any questions, reply directly to this email.
          </p>
          <p style="margin:12px 0 0;font-size:11px;color:#9CA3AF;word-break:break-all;">${url}</p>
          <table cellpadding="0" cellspacing="0" style="margin:28px 0 0;border-top:1px solid #E5E7EB;width:100%;">
            ...
            VaughnMartin · Readiness OS<br/>pilot@vaughnmartin.com
              ...
              for (const from of fromAddresses) {
                const { error } = await resend.emails.send({
                  ...
                  subject: `Your Readiness OS Access — Valid for ${expiryLabel}`,
                  html,
                });
                if (!error) {
                  emailSent = true;
                  console.log(`[GroupLink] Email sent via ${from} to ${recipientEmail}`);
                  break;
                } else {
                  console.warn(`[GroupLink] Send failed from ${from}: ${error.message}`);
                  emailError = error.message;
                }
              }
              if (!emailSent) console.warn(`[GroupLink] All from-addresses failed — copy link manually`);
            } catch (err: any) {
              emailError = err.message;
              console.warn(`[GroupLink] Email threw: ${err.message}`);
            }
          }
        }
      }
