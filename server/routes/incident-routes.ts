...
Return ONLY raw JSON (no markdown, no code fences):
{
  ...
  "matched_playbook": {"code": "#MKT-001 or #SEC-001 or #TRN-001", "name": "Matched playbook name from 180 library", "exists_in_library": true},
  ...
  try {
    ...
    if (domain === 'offense') {
      ...
      if (answers.playbookLocation === "We don't have one") {
        gaps.push("No documented market entry playbook"); recommendations.push("Use Readiness OS's 58 Offense playbooks to build your market entry protocols");
      ...
      if (answers.playbookLocation === "We don't have one" || answers.playbookLocation === "Don't have one") {
        gaps.push("No documented response playbook exists"); recommendations.push("Use Readiness OS's 56 Defense playbooks to build your crisis response protocols");
      } else if (answers.playbookLocation) {
        score += 20;
      ...
      else { gaps.push("No pre-defined RACI matrices for transformation scenarios"); recommendations.push("Build RACI matrices for your top transformation scenarios using Readiness OS templates"); }
      ...
      if (answers.playbookLocation === "We don't have one") {
        gaps.push("No documented transformation playbook"); recommendations.push("Use Readiness OS's 52 Special Teams playbooks for transformation initiatives");
      } else if (answers.playbookLocation) {
        score += 15;
        if (['Confluence/SharePoint', 'Spreadsheets'].includes(answers.playbookLocation)) {
