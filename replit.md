...
- **Navigation:** Streamlined nav. Unauthenticated CTAs: "Try Demo" (outline, → /try-demo), "Request Pilot" (gold primary, → /pilot-program), "Sign In" (ghost). Authenticated: "Try Demo" + "Request Pilot" always visible (same as unauthenticated — executives need these to share with prospects), "Open Platform" (teal, → /mission-control), user name as a dropdown containing Settings (→ /settings), Organization Setup (→ /organization-setup), and Sign Out. "Start Free Trial" removed — enterprise B2B only. **Rule: No user should ever need to type a URL — every page must be reachable through the UI.**
...
- **Onboarding Guard:** Frontend `OnboardingGuard` component in `App.tsx` redirects new users to `/onboarding` **once only** — uses a `useRef` flag (`hasRedirected`) so the redirect fires a single time per session, then the user can navigate freely. Do NOT revert to the old pattern of redirecting on every navigation event — that trapped users on the onboarding page. The onboarding wizard has a "Skip to Platform →" button in the step view header and a "Skip for now" link in the journey view, both calling `completeOnboardingMutation` which marks `onboardingCompleted: true` and invalidates the auth cache.
...
- **Enriched Playbook Content (March 2026):** ALL 170 playbooks now have 6 enriched content fields on the `playbook_library` table: `why_it_matters`, `signal_sources`, `enriched_phases` (4-phase breakdown with role-specific tasks, deadlines, restrictions, and decision gates), `communication_assets` (board notifications, customer outreach scripts, sales battle cards), `risk_indicators` (green/yellow/red), `outcome_framing` (12-hour and 30-day success criteria + failure modes). `PlaybookDetail.tsx` renders all enriched sections. The 3 free sample playbooks (`enrichPlaybookSamples.ts`) were hand-crafted; the remaining 167 were AI-generated via `server/seeds/enrichAllPlaybooks.ts` using gpt-5.2 with 3-concurrent batching — 170/170 complete, 0 failures, verified across all 9 domains.
