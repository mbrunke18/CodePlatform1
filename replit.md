...
- **Enriched Playbook Content (March 2026):** All 3 free sample playbooks now have 6 enriched content fields on the `playbook_library` table: `why_it_matters`, `signal_sources`, `enriched_phases` (4-phase breakdown with role-specific tasks, deadlines, restrictions, and decision gates), `communication_assets` (board notifications, customer outreach scripts, sales battle cards), `risk_indicators` (green/yellow/red), `outcome_framing` (12-hour and 30-day success criteria + failure modes). `PlaybookDetail.tsx` renders all enriched sections. Seed script: `server/seeds/enrichPlaybookSamples.ts`.
