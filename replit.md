...
- **FATHER'S ORIGIN STORY (LOCKED — Never Remove):** "Prepare. Practice. Perform fearless. Never give up." — words from the founder's father on the first day he played football in third grade. These four words are the origin of the company name VaughnMartin (named for the family that gave the framework), the product thesis (Preparation → Readiness → Fearless), and the platform arc. Deployed as "The Origin" prologue section at the top of FounderStory.tsx, appearing before Section I. The sequence — prepare, practice, perform fearless, never give up — maps directly to the IDEA Framework and every playbook in the system. Never remove or rewrite this section.
- **DR. KERRY HUANG FRAMEWORK (LOCKED — Core Intellectual Foundation):** ESI Top 1% Researcher, Forbes Business Council, 408-firm study. Key findings now embedded in the platform:
  ...
  — **Ownership Confirmation (LOCKED):** Ownership is NOT a behavioral outcome of good preparation, NOR a state of mind. It is a concrete, verifiable record — something the preparation phase either produces or fails to produce. The acknowledgment step makes that record visible in real time. Assignment = naming someone. Ownership = that person challenged the playbook, rehearsed the decision, signed off before pressure existed. NOTE: The phrase "Ownership as Artifact" originated in a private exchange with Dr. Huang and is NOT used in any visible UI copy until he approves its public use. All UI copy uses "ownership record," "ownership confirmed," or "confirm ownership" instead.
  — **"Silence at Acknowledgment" (LOCKED):** The earliest signal that preparation didn't transfer is not that the playbook failed — it is silence at acknowledgment. Not silence at completion. Not divergence at debrief. Silence at the moment the response either deploys or does not, in the first minutes. This is Martin's own concept, confirmed by Huang.
  ...
  — Deployed in: FounderStory ("Built vs. Received" interstitial + Research Anchor section + Deep Thesis pullquote), PlaybookCustomize (Challenge Rights + First-Minute Test section), PlaybookActivationConsole (Ownership Close-Out Gate + "Silence at acknowledgment" diagnostic line), PlaybookActivationConsole ADVANCE debrief (Recovery vs. Optimization Classification), InvestorLanding (Competitive Moat Research Foundation callout).
...
- **FOUNDER STORY VISIBILITY:** "The Manifesto" appears in HomepageNav desktop and mobile (styled italic Cormorant Garamond to signal personal/editorial). Homepage founder quote block links to `/founder-story` via: "Why this company exists — and why it's named what it is →". The story is mid-funnel discovery (after product hook), not the lead.
...
- **Typography:** Global base font-weight 500; headings are font-weight 700 in midnight navy. Cormorant Garamond for editorial sections, Barlow Condensed for section labels.
...
- **Ownership Close-Out Gate:** Formal governance verdict at activation completion inside PlaybookActivationConsole. Measures ownership transfer rate, surfaces silent tasks diagnostic, delivers one of three verdicts: Ownership Confirmed (≥70%), Partial Transfer (35-69%), Silence Detected (<35%). Includes three-signal test and Dr. Huang attribution.
- **Recovery vs. Optimization Debrief Classification:** ADVANCE debrief inside PlaybookActivationConsole automatically classifies itself based on Close-Out Gate ownership %: Optimization (≥70%), Mixed-Signal (35-69%), Recovery (<35%). Each type has distinct instructions, focus questions, and a Dr. Huang quote about the acknowledgment window.
