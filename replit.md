...
- **PRODUCT THESIS ARC (LOCKED):** Preparation → Readiness → Fearless. "Any organization can be ready and prepared to respond to any situation they would expect to encounter or have encountered." The canonical tagline is: **"When the situation arrives — The Response Is Ready Before the Trigger Fires."** (three-line visual display: italic setup line "When the Situation Arrives —" in white/muted, then "The Response Is Ready" in white, then "Before the Trigger Fires." in gold) The emotional endpoint of the platform is fearlessness — not speed. Speed is the evidence; readiness is the promise; fearless is the outcome. This arc must be present on the Homepage hero, Investor pages, WelcomeBrief, TwelveMinuteTestDrive debrief, and FounderStory. Every enterprise that prepares for every situation it'll face is no longer afraid of strategic triggers — it's fearless.
...
- **FOUNDING PARTNER PROGRAM (LOCKED):** The pre-launch program is "Founding Partner Program" — never "Pilot Program," "Pilot Access," or "Now in Pilot" in user-facing copy. The program is a 90-day validation partnership. Button labels: "Apply for Founding Partner Access" or "Request Founding Partner Access." CTAs pointing to /contact or /request-access must use Founding Partner language. This rule applies to visible UI text only — internal variable names or route slugs are exempt.
...
- **CANONICAL PRODUCT NARRATIVE (LOCKED — use as the authoritative verbiage reference for all copy, descriptions, and story arcs):** "Pre-staged before the trigger. Authorized in real time. Teams mobilized in 12 minutes — and executing from a fully-staged position. Every activation makes the next response faster." This four-sentence sequence captures the complete product story in order: (1) preparation before the event, (2) human authorization in the moment, (3) the 12-minute mobilization outcome, (4) the continuous improvement loop (ADVANCE 2.0). When writing hero copy, section intros, investor decks, or onboarding text, test it against this sequence to ensure the verbiage is consistent and trustworthy.
- **DESCRIPTOR TERMINOLOGY HIERARCHY (LOCKED):** "Situational Readiness Platform" is RETIRED from all user-facing copy — it was generic and disconnected from the locked messaging. One primary descriptor plus two contextual variants: (1) **Primary — "Readiness Infrastructure"**: used everywhere as the product descriptor — nav subtitles, hero badges, meta titles, footers, investor pages. Pairs directly with "Readiness OS." (2) **Contextual — "Coordination Infrastructure"**: used only in competitive positioning contexts (CompetitivePositioning page, partner briefs, category-of-one framing). (3) **Contextual — "Operating Model Layer"**: used only when describing where Readiness OS sits above the Microsoft/AI stack in investor and integration diagrams. Never mix these — the primary is "Readiness Infrastructure."
- **CONCEPT HIERARCHY (LOCKED):** "Mobilization is work" is the structural insight underneath everything — every enterprise treats mobilization (assembling authority, teams, budget, sequencing, systems, communication, compliance, governance record, learning) as overhead instead of something engineered; Readiness OS is the first platform to engineer it. "The 12 Mobilization Gaps" (Detection, Recognition, Authority, Team Assembly, Budget, External Resources, Sequencing, Systems, Communication, Compliance, Governance Record, Learning) are the diagnostic layer — the specific, nameable failure points inside the mobilization-is-work insight, used on `/the-gap` and in sales/investor conversations to let a prospect self-diagnose rather than be pitched. The SaaS-contrast framing — "Most enterprise software: a better way to do existing work. Readiness OS: a new category based on a structural organizational flaw that has never had a platform." — is the category-argument layer that sits above both, used on `/investors` and investor materials. Keep the three distinct: insight → diagnostic → category argument.
...
- **Terminology Enforcement:** Consistent use of "12 minutes," "3,600×," "180 Readiness Protocols," and "231 triggers." "Readiness Protocol" is the canonical term for all execution artifacts.
...
- **PMO Director Onboarding (`/pmo-onboarding`):** Persona-specific onboarding path for the preparation architecture owner. Covers the 3-tier ownership model (C-suite = Authorization, PMO = Preparation Architecture, Functional = Execution), 4-phase go-live path with explicit PMO tasks per phase, and weekly/monthly/quarterly governance rhythm. Accessible from StandardNav under "Inside the Platform."
...
- **Authorization Precedent Panel:** Shown in `GovernanceReadinessCheck.tsx` at the exact moment of executive decision — above the 3 pre-flight questions. Displays prior authorization records (executive initials, date, choice made, outcome: Proven teal / Measuring amber) so the authorizing executive can see how peers decided in identical situations. Records will be sourced from live execution + close-out data; currently seeded with illustrative records.
- **Capability Survivability Indicator:** Badge on every protocol card in `ProtocolLibrary.tsx`. Three states derived from stakeholder count and activation history — System-Embedded (teal, ≥3 stakeholders + ≥3 activations), Establishing (gold, partial), Owner-Dependent (gray, low configuration). Tooltip explains survivability meaning. Appears below the evolution indicators row.
