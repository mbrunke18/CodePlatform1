...
- **Branding:** Consistent `VaughnMartin` and `Execution OS` branding with logo placement on all key pages. **Logo is the circular seal** (`ExecuteIQLogo.tsx` / `VaughnMartinLogo`): gold gradient rings (3.5px outer), 8 tick marks (4 cardinal + 4 diagonal), "VAUGHNMARTIN" top arc + "EXECUTION OS" bottom arc in Barlow Condensed, VM monogram, gold dot with rays, diamond ornaments. Uses React `useId` for unique gradient IDs per instance. Variants: `full` | `icon-only` | `text-only`. Colors: `navy` (gold seal on light bg) | `white`/`teal` (gold seal on dark bg) | `gold` (navy seal on gold bg). Two-brand architecture: **VaughnMartin** = the institution ("The layer nobody built") | **Execution OS** = the product ("We Make Enterprises Fearless").
