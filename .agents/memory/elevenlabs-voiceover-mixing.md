...
## Per-segment onset variance across a TTS batch — pad content, don't keep re-tuning the fade
Even after picking a safely-short fade (e.g. 20ms) based on one segment's onset, a full batch of ~19 TTS takes from the same voice/model can still have per-segment onset variance from ~15ms to ~120ms (driven by which phoneme a line starts with — sibilants like "System" read hot earlier than plosives). A single measured segment is not representative of the whole batch; one outlier with an unusually early onset (~15ms) can still get its first phoneme partially ramped-through by a fade tuned safe for other segments, and is heard as a missing/clipped first word on that one line only — easy to miss if only spot-checking a couple of segments.
Fix at the content level, not the fade level: insert a uniform silence pre-pad (`adelay=50|50`) on every segment's own filter chain *before* the existing fade filter (e.g. `[N:a]adelay=50|50,${FADEIN},adelay=<abs_ms>|<abs_ms>[aN]`), so the fade always ramps across guaranteed pure silence regardless of that take's natural onset. This leaves the fade duration/mechanism untouched (don't re-litigate it) and only shifts real speech later by a fixed, imperceptible amount inside each segment's already-existing inter-segment gap.

**Why:** A user-reported "first word missing at the start of every scene" persisted after the fade duration itself was already correct (20ms) and verified fine for most segments — the actual cause was one specific take with a 14.8ms onset, narrower than the fade window. Padding all segments uniformly (rather than hand-tuning per-segment fade timing) fixed it without reopening the fade-duration question.

**How to apply:** Before shipping a TTS voiceover batch, run the astats/silencedetect onset check across *every* segment (not just one or two) and note the minimum. If the minimum onset is close to or under the fade duration, add a uniform `adelay` pre-pad ahead of the fade on all segments rather than shortening the fade further or hand-tuning one outlier.

## CORRECTION: the fade/pad theory above was WRONG for persistent "first word missing" on scene-opening lines — true cause is ElevenLabs cold-start truncation, not a post-processing fade/onset issue
After shipping the pad+short-fade fix above, the user reported the cutoff STILL happened, but only on specific "first-in-scene" lines (the very first TTS segment played in each cut/scene — hook lines, intro, endcard opener). Measuring onsets on those exact segments showed 60-112ms — comfortably past the 20ms fade and even the 50ms pad — proving the audio file itself was missing/clipping real phoneme content from its very first generated sample, not a downstream mixing artifact. This is a known ElevenLabs artifact: the model can truncate/clip the leading phoneme(s) of a generation when the target text is the literal first thing spoken (a "cold start"), independent of any ffmpeg processing.
**Fix:** generate the segment with a short disposable spoken lead-in phrase prepended to the real text (e.g. `"One moment. " + realText`), then trim the lead-in out of the resulting audio afterward — silence-gap-detect the pause after the lead-in (`ffmpeg -af silencedetect=noise=-35dB:d=0.08`; the lead-in's own pause consistently showed up as the *first* detected silence gap, ~0.5-0.6s in, across every affected segment) and re-encode from a point ~30-40ms before that gap's `silence_end` (preserves a small clean silence buffer for the existing fade). The real target word is then never the literal cold-start sample of the generation, and its true onset (unaffected by the artifact) plays normally.
**Recovering exact original narration text for regeneration:** if the original script text isn't saved anywhere in the repo/build script (e.g. hook lines only exist as differently-worded ALL-CAPS captions, not verbatim narration), transcribe the existing approved audio first rather than guessing/rewriting — reconstructing "close enough" wording risks silently changing approved narration content. `whisper-1` was unavailable both directly (OpenAI key had `insufficient_quota`) and via the Replit AI Integrations OpenAI proxy (`UNSUPPORTED_MODEL`); `gpt-4o-transcribe` via the same Replit AI Integrations proxy (`${AI_INTEGRATIONS_OPENAI_BASE_URL}/audio/transcriptions`) worked and returned accurate verbatim text.

**Why:** The pad/fade fix (previous section) was diagnosed from onset timing data that looked identical for affected and unaffected segments — it fixed a real but different problem (fade-ramping through an early-but-present onset) while leaving a separate, deeper problem (content actually missing from the file) undiagnosed. Only isolating and listening/transcribing the specific complained-about segments (not a random sample) revealed the true cause.

**How to apply:** If "first word cut off" complaints persist after confirming fade duration and pre-pad are already correct, suspect TTS-side generation truncation instead of mixing — especially for whichever segment plays first in its scene/file. Regenerate with a disposable lead-in + trim rather than further tuning ffmpeg fade/pad parameters.

## ElevenLabs can garble specific 4-digit numbers with a leading low digit (e.g. "1,400") — verify pronunciation with a special transcription prompt, not the default one
A number written as digits in TTS input text (e.g. "1,400") is not guaranteed to read naturally. One case produced "one four hundred" — neither the natural spoken form ("fourteen hundred") nor the formal one ("one thousand four hundred") — an audibly broken reading. Standard `gpt-4o-transcribe` transcription silently normalizes any spoken number back to digits ("1400"), so a normal transcript looks identical whether the audio was correct or garbled — it cannot be used to detect this class of bug.
**Detection:** pass a transcription prompt that explicitly forbids digit output, e.g. "Transcribe the exact words spoken, phonetically spelling out any numbers exactly as pronounced. Do not use digits anywhere in your output." This forces the transcript to reveal the actual spoken words, exposing the garble.
**Fix:** rewrite the source text with the number spelled out the way a human would actually say it (e.g. "Fourteen hundred") rather than leaving it as digits, then regenerate.
**Verified safe patterns (no rewrite needed):** plain 2-3 digit numbers (23, 31, 89, 180, 231, 380) and round thousands-with-hundreds numbers (3,600 → "three thousand six hundred") all read correctly as digits with this same voice/model.
