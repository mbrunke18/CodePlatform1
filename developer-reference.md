...
## 43. A16Z SpeedRun Pitch Deck — April 24, 2026 (rev 27)

### Overview

A standalone 13-slide investor pitch deck built for the a16z SpeedRun 007 application. Lives at two routes:
- `/a16z` — primary route
- `/speedrun-pitch` — alias

**File:** `client/src/pages/A16ZPitch.tsx`
**Registered in:** `client/src/App.tsx` (lazy imported at both routes)
...
The deck renders as a fixed **960×540px** canvas (true 16:9 / PowerPoint standard) that CSS-scales to fill the viewport while preserving aspect ratio. This is not a responsive layout — it is a fixed-pixel canvas scaled by a computed `scale` factor:

```ts
const scale = Math.min(window.innerWidth / 960, (window.innerHeight - NAV_H) / 540);
```

The canvas div uses `overflow: hidden` — any content exceeding 540px height is silently clipped. This is the most common failure mode when adding new content to slides. Always calculate whether total content height fits within 540px before adding elements.

**Nav bar height:** `NAV_H = 44px` (subtracted from available viewport height before scale calculation)

**Critical layout rule:** Outer padding on full-height slides should not exceed `28px` top/bottom. Slides with `padding: "52px"` or greater on content-heavy layouts will overflow and clip content at the bottom.

---

### Slide Inventory (13 slides)

| # | Component | Label | Background | Key Content |
|---|---|---|---|---|
| 1 | `CoverSlide` | Cover | `NAVY_BG` dark | Tagline, Martin Brunke, pilot@ email |
| 2 | `ProblemSlide` | The Problem | White + dark left panel | 30-day stat, real-world trigger costs, 8 trigger types |
| 3 | `InsightSlide` | The Insight | `OFF` | Fortune 1000 vs Championship Football comparison grid |
| 4 | `ProductSlide` | The Product | White | IDEA Framework grid, 3 strategic domains, stat bars |
| 5 | `PlatformDepthSlide` | Platform Depth | `OFF` | 6 feature cards: Command Tower, War Room, Readiness Score, Shadow Simulator, Regulatory Calendar, Integration Hub |
| 6 | `ValidationSlide` | Validation | `OFF` | 4 researcher cards: Huang, DeJarnette, Highsmith, Suri |
| 7 | `WhyNowSlide` | Why Now | `NAVY_BG` dark | 3 columns: Decision Density, Microsoft Gap, Category Window |
| 8 | `MarketSlide` | The Market | White + dark left panel | TAM math ($500M+ SAM), Microsoft distribution wedge |
| 9 | `MoatSlide` | The Moat | White | 3 moat cards: Protocol Library, Governance Research, Operating Model |
| 10 | `ModelSlide` | The Model | `OFF` | Enterprise Pilot ($75K) + Growth Deployment tiers |
| 11 | `TractionSlide` | Traction | White | Product metrics, validation signals, SpeedRun unlock |
| 12 | `FounderSlide` | The Founder | `NAVY_BG` dark | Martin Brunke bio, football origin story reference |
| 13 | `CloseSlide` | Close | `NAVY_BG` dark | "We make enterprises fearless." CTA + contact |

---

### Cost-Per-Trigger Figure (Problem Slide)

**Current value:** `$5M – $575M+`
**Label:** "Real-world trigger costs"
**Sources documented on the slide:**
- Equifax breach: $575M FTC settlement (2017)
- Target cyber incident: $292M total cost (2013)
- Boeing 737 MAX: $20B+ total impact
- M&A integration failure average: $277M in destroyed value (McKinsey research)

**Do not revert to the previous unsourced range "$50M–$500M."** The current figure is defensible from named public cases. If this range is updated in the future, every number used must be traceable to a named company or named research study — no ranges without sources.

---

### PPTX Export

The deck includes a browser-side PowerPoint export. No server involved.

**Dependencies added:**
- `html2canvas` — renders the 960×540 DOM canvas to a bitmap
- `pptxgenjs` — packages bitmaps into a `.pptx` file

**How it works:**
1. User clicks "Download PPTX" in the bottom nav bar
2. A navy overlay with a gold progress bar appears (slide N of 13)
3. For each slide 0–12: `setCurrent(i)` → 350ms delay (render settle) → `html2canvas` capture at `scale: 2` (1920×1080 effective resolution) → JPEG at 95% quality
4. `PptxGenJS` assembles 13 slides with `LAYOUT_16x9` (10" × 5.625")
5. Each slide image is placed at `x:0, y:0, w:10, h:5.625` (full bleed)
6. `.writeFile()` triggers browser download as `VaughnMartin-ReadinessOS-a16z-SpeedRun007.pptx`
7. Deck resets to original slide; overlay dismisses

**Known behavior:** The export sequentially flips through all 13 slides at ~350ms each (~5 seconds total). The overlay prevents user interaction during this window. Web fonts (Cormorant Garamond, Barlow Condensed) render correctly because they are already loaded by the browser at capture time.

**Button location:** Bottom nav bar, right side, gold-bordered with Download icon. Shows `Slide X/13` count while exporting; reverts to "Download PPTX" when complete.

---

### Navigation

- **Keyboard:** `ArrowRight` / `ArrowDown` advance; `ArrowLeft` / `ArrowUp` go back
- **Dot indicators:** Bottom center — clickable, gold pill on active slide
- **Chevron buttons:** Left/right edges, fixed position, fade at deck boundaries

---

### Design Notes

- Uses the same brand constants as the rest of the platform: `NAVY`, `NAVY_BG`, `GOLD`, `TEAL`, `OFF`, `BORDER`
- Typography: `CG` (Cormorant Garamond) for editorial text; `BC` (Barlow Condensed) for labels/stats
- All slide components are self-contained functions — no shared state between slides
- The `SlideLabel`, `GoldRule` shared components from `client/src/components/design/index.tsx` are NOT used — the deck defines its own local `SlideLabel` and `GoldRule` functions at the top of `A16ZPitch.tsx` to keep the file fully standalone

---

### Adding or Modifying Slides

1. Write a new slide function (e.g., `function NewSlide() { ... }`) following the existing pattern
2. The outer div must be `width: "100%", height: "100%"` — the canvas sizes it to 960×540
