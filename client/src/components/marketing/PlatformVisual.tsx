        ...
        <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr_200px] gap-6 items-start mb-16">
          ...
          >
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">IDEA Framework™</p>
            </div>
              ...
              {ideaPhases.map((phase, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    ...
                    <span className="text-[10px] text-poise-teal font-medium">AI</span>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 ml-1">{phase.subtitle}</span>
                  </div>
                ...
                <ul className="space-y-1">
                  {output.items.map((item, j) => (
                    <li key={j} className="flex items-start gap-1.5">
                      ...
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">{item}</span>
                    </li>
                  ))}
                </ul>
              </motion.div>
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {integrations.map((integration) => (
                ...
                >
                  <integration.icon className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" />
          ...
          >
            ...
            <div className="flex flex-wrap gap-1.5">
              {domains.map((domain) => (
                <span
                  key={domain}
                  className="text-[10px] px-2 py-1 rounded-md bg-poise-gold/10 text-poise-gold border border-poise-gold/20 font-medium"
                >
                  {domain}
                </span>
              ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-4">
            {[
              ...
              <div key={capability.title} className="text-center">
                ...
                <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">{capability.desc}</div>
              </div>
            ))}
          </div>
        </motion.div>
              ...
              <div key={step.step} className="flex items-center">
                <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500 dark:text-slate-400">{step.desc}</div>
                  </div>
                </div>
                {i < customerJourney.length - 1 && (
                  <ArrowRight className="h-4 w-4 text-slate-300 dark:text-slate-600 mx-1 hidden md:block" />
                )}
              </div>
            ))}
          </div>
