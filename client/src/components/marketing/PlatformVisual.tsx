        ...
        <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr_200px] gap-6 items-start mb-16">
          ...
          >
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-slate-400 dark:text-slate-300">IDEA Framework™</p>
            </div>
              ...
              {ideaPhases.map((phase, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    ...
                    <span className="text-xs text-slate-400 dark:text-slate-300 ml-1">{phase.subtitle}</span>
                  </div>
                ...
                <ul className="space-y-1">
                  {output.items.map((item, j) => (
                    <li key={j} className="flex items-start gap-1.5">
                      ...
                      <span className="text-xs text-slate-400 dark:text-slate-300 leading-tight">{item}</span>
                    </li>
                  ))}
                </ul>
              </motion.div>
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {integrations.map((integration) => (
                ...
                >
                  <integration.icon className="h-3.5 w-3.5 text-slate-400 dark:text-slate-300" />
                  <span className="text-xs text-slate-400 dark:text-slate-300 font-medium">{integration.name}</span>
                </span>
              ))}
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-4">
            {[
              ...
              <div key={capability.title} className="text-center">
                ...
                <div className="text-xs text-slate-400 dark:text-slate-300 mt-0.5">{capability.desc}</div>
              </div>
            ))}
          </div>
        </motion.div>
              ...
              <div key={step.step} className="flex items-center">
                <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-400 dark:text-slate-300">{step.desc}</div>
                  </div>
                </div>
                {i < customerJourney.length - 1 && (
                  <ArrowRight className="h-4 w-4 text-slate-600 dark:text-slate-200 dark:text-slate-300 mx-1 hidden md:block" />
                )}
              </div>
            ))}
          </div>
