  ...
  return (
    <PageLayout>
      <div style={{ background: "#fff", minHeight: "100vh" }}>
        ...
        {/* ── Advisory Board ──────────────────────────────────────────────────── */}
        <section style={{ background: "#fff", padding: "80px 32px", borderTop: `1px solid ${NAVY}10` }}>
          <div style={{ maxWidth: 960, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 48 }}>
              <div style={{ height: 1, width: 28, background: GOLD }} />
              <span style={{ ...DM, fontSize: 10, fontWeight: 700, letterSpacing: "0.28em", textTransform: "uppercase", color: GOLD }}>Advisory Structure</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 64, alignItems: "start" }}>
              <div>
                <h2 style={{ ...CG, fontSize: "clamp(26px,2.8vw,38px)", fontWeight: 700, color: NAVY, lineHeight: 1.1, margin: "0 0 20px" }}>
                  The advisory board is purpose-built for this category.
                </h2>
                <p style={{ fontSize: 15, color: `${NAVY}70`, lineHeight: 1.8, fontWeight: 500, marginBottom: 28 }}>
                  VaughnMartin's advisory structure connects research validation, enterprise domain expertise, and go-to-market authority across the company's three primary buyer segments: Fortune 500 C-suite, PE-backed mid-market, and regulated industries.
                </p>
                <div style={{ borderLeft: `2px solid ${GOLD}50`, paddingLeft: 20 }}>
                  <p style={{ ...CG, fontSize: 16, fontStyle: "italic", color: `${NAVY}80`, lineHeight: 1.7, margin: 0 }}>
                    "The research confirms what the platform operationalizes: the organizations that prepared in advance moved without fear. The ones that didn't were still aligning when the window closed."
                  </p>
                  <p style={{ ...DM, fontSize: 11, fontWeight: 700, color: `${NAVY}50`, marginTop: 10, letterSpacing: "0.08em" }}>
                    Martin Brunke — Founder & CEO
                  </p>
                </div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 1, background: `${NAVY}10` }}>
                {[
                  {
                    initials: "KH",
                    name: "Dr. Kerry Huang",
                    title: "Research Advisory",
                    creds: "ESI Top 1% Researcher · Forbes Business Council · Fortune 50 AVP · 408-Firm Governance Study",
                    note: "Independent academic validation. His 408-firm study established the empirical foundation for Readiness OS's core thesis: preparation produces ownership. LinkedIn public validation, April 2026.",
                    accent: TEAL,
                  },
                  {
                    initials: "EA",
                    name: "Enterprise Architecture Advisor",
                    title: "Technical Advisory — Recruiting",
                    creds: "Former CTO or VP Engineering · Fortune 500 digital transformation · Microsoft stack depth",
                    note: "Validates the technical architecture and Microsoft integration layer for enterprise procurement conversations.",
                    accent: GOLD,
                  },
                  {
                    initials: "CS",
                    name: "Chief Security Advisor",
                    title: "Security & Compliance Advisory — Recruiting",
                    creds: "Former CISO · Regulated industry (financial services, healthcare, or energy) · Board-level experience",
                    note: "Anchors the RISK & RESILIENCE protocol library and the security compliance positioning for regulated-industry buyers.",
                    accent: NAVY,
                  },
                  {
                    initials: "GS",
                    name: "Go-to-Market Advisor",
                    title: "Sales & Distribution Advisory — Recruiting",
                    creds: "VP Sales or CRO · Enterprise SaaS (Salesforce, ServiceNow, or equivalent) · $50M+ ARR experience",
                    note: "Anchors the enterprise direct sales motion and channel partnership strategy for the Fortune 500 segment.",
                    accent: TEAL,
                  },
                ].map(({ initials, name, title, creds, note, accent }) => (
                  <div key={initials} style={{ background: "#fff", padding: "28px 32px", display: "flex", gap: 24, alignItems: "flex-start" }}>
                    <div style={{
                      width: 48, height: 48, borderRadius: "50%", flexShrink: 0,
                      background: accent === NAVY ? `${NAVY}15` : accent,
                      display: "flex", alignItems: "center", justifyContent: "center",
                    }}>
                      <span style={{ ...CG, fontSize: 16, fontWeight: 700, color: accent === NAVY ? NAVY : "#fff" }}>{initials}</span>
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 4 }}>
                        <span style={{ ...CG, fontSize: 17, fontWeight: 700, color: NAVY }}>{name}</span>
                        <span style={{ ...DM, fontSize: 10, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: accent === NAVY ? `${NAVY}55` : accent }}>{title}</span>
                      </div>
                      <div style={{ ...DM, fontSize: 11, fontWeight: 600, color: `${NAVY}55`, marginBottom: 8, letterSpacing: "0.04em" }}>{creds}</div>
                      <div style={{ fontSize: 13, color: `${NAVY}65`, lineHeight: 1.7, fontWeight: 500 }}>{note}</div>
                    </div>
                  </div>
                ))}
              </div>
