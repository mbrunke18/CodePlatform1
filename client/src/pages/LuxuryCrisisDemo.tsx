...
const TASKS = [
  ...
  { phase: "PARTNER BRIEFING", role: "Chief Revenue Officer", action: "Brief top wholesale partners and all 4,200 retail points of sale — equip floor staff with customer response protocol before doors open", time: "8:00", priority: "high" },
        ...
        {step === 1 && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.68)', maxWidth: 560, margin: '0 auto' }}>{SCENARIO.company}</p>
            </div>
            ...
            <div style={{ padding: '24px 28px', background: 'rgba(192,57,43,0.08)', border: '1px solid rgba(192,57,43,0.25)', borderLeft: '4px solid #C0392B', marginBottom: 32 }}>
              <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#f87171', marginBottom: 10 }}>● TRIGGER ACTIVE — {SCENARIO.urgency.toUpperCase()}</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 16, padding: '10px 16px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', marginBottom: 24 }}>
              ...
              {[{ color: '#D1D5DB', bg: 'transparent', label: 'Queued' }, { color: GOLD, bg: 'rgba(201,168,76,0.2)', label: 'Notified' }, { color: TEAL, bg: TEAL, label: 'Acknowledged ✓' }].map(s => (
                <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 11, color: s.label === 'Acknowledged ✓' ? TEAL_LT : s.label === 'Notified' ? GOLD : 'rgba(255,255,255,0.68)', fontWeight: 600 }}>{s.label}</span>
                </div>
              ))}
            </div>
        ...
        {step === 4 && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 40 }}>
              {[
                ...
                <div key={m.label} style={{ padding: '20px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderTop: `3px solid ${m.color}`, textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.68)' }}>{m.sub}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ padding: '24px 32px', background: 'rgba(43,138,110,0.08)', border: `1px solid rgba(43,138,110,0.3)`, borderLeft: `4px solid ${TEAL}`, marginBottom: 40, textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.72)', letterSpacing: '0.06em' }}>That's preparation. That's readiness. That's how enterprises become fearless.</p>
            </div>
            <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
              <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.65)', maxWidth: 480 }}>Ready to pre-stage this for your organization — with your real Maisons, your real VIP client list, and your real Readiness Protocols?</p>
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
                <a href="/founding-partner-program" style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '13px 32px', background: GOLD, color: NAVY, textDecoration: 'none' }}>Request Founding Partner Access →</a>
