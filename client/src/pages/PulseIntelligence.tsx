      ...
      <PageLayout>
        <div className="min-h-screen bg-slate-950 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-slate-400">Unable to load signal intelligence data. Please try again shortly.</p>
          </div>
        </div>
      </PageLayout>
    );
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm">AI-powered threat detection, analysis, and strategic response orchestration</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-[10px] text-slate-500">{timeAgo(item.detectedAt)}</span>
                            <span className="text-[10px] text-slate-600">·</span>
                            <span className="text-[10px] text-slate-500 truncate">{item.signalSource.split('+')[0].trim()}</span>
                          </div>
                        ...
                        <ChevronRight className={`h-4 w-4 flex-shrink-0 mt-1 transition-colors ${isSelected ? 'text-indigo-400' : 'text-slate-600'}`} />
                      </div>
                    </button>
                  );
                })}
                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs text-slate-400">{selected.signalSource}</span>
                            <span className="text-xs text-slate-600">·</span>
                            <span className="text-xs text-slate-400">Detected {timeAgo(selected.detectedAt)}</span>
                          </div>
                        </div>
                      </div>
                    </CardHeader>
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">
                        {selected.dataPoints.map((dp, i) => (
                          <div key={i} className="p-2.5 bg-slate-800/40 rounded-md border border-slate-700/30">
                            <div className="text-[10px] text-slate-500 uppercase tracking-wider">{dp.label}</div>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 mt-3">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs text-slate-400">Risk: {selected.aiAnalysis.riskLevel}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-xs text-slate-400">{selected.recommendedPlaybook.domain}</span>
                            <span className="text-xs text-slate-600">·</span>
                            <span className="text-xs text-slate-400">{selected.recommendedPlaybook.tasksCount} pre-configured tasks</span>
                          </div>
                        </div>
                      </div>
              ...
              {selected && (
                <>
                  ...
                  <Card className="bg-slate-900/40 border-slate-800/50">
                    <CardContent className="p-4">
                      <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Signal-to-Action Flow</div>
                      <div className="space-y-0">
                        {[
                          ...
                          { label: 'Execution Started', detail: getDecisionStatus(selected) === 'approved' ? 'In progress' : 'Pending decision', icon: <Zap className="h-3.5 w-3.5" />, color: getDecisionStatus(selected) === 'approved' ? 'text-emerald-400' : 'text-slate-500', bgColor: getDecisionStatus(selected) === 'approved' ? 'bg-emerald-500/20' : 'bg-slate-800', done: getDecisionStatus(selected) === 'approved' },
                        ].map((step, i) => (
                          <div key={i} className="flex items-start gap-3">
                            ...
                            <div className="pb-4">
                              <div className={`text-xs font-medium ${step.done ? 'text-white' : 'text-slate-400'}`}>{step.label}</div>
                  ...
                  <Card className={`border ${getDecisionStatus(selected) === 'approved' ? 'bg-emerald-950/20 border-emerald-500/30' : getDecisionStatus(selected) === 'rejected' ? 'bg-slate-900/40 border-slate-700/30' : 'bg-amber-950/20 border-amber-500/30'}`}>
                    <CardContent className="p-4">
                      {getDecisionStatus(selected) === 'approved' ? (
                        ...
                        <div className="text-center py-2">
                          <XCircle className="h-8 w-8 text-slate-500 mx-auto mb-2" />
                          <div className="text-sm font-semibold text-slate-400">Action Declined</div>
                          <div className="text-xs text-slate-500 mt-1">Signal monitored - no playbook activation</div>
                          <Button size="sm" variant="outline" className="mt-3 border-slate-700 text-slate-400" onClick={() => handleDecision(selected.id, 'pending')}>
                            Reconsider
                          </Button>
                        </div>
                      ) : (
          ...
          <div className="mt-6 p-4 bg-gradient-to-r from-indigo-950/40 via-purple-950/40 to-indigo-950/40 border border-indigo-500/20 rounded-xl">
            <div className="grid grid-cols-4 gap-6 text-center">
              <div>
                ...
                <div className="text-xs text-slate-400">Data Points Monitored 24/7</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-white">12 min</div>
                <div className="text-xs text-slate-400">Signal to Execution</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-white">166</div>
                <div className="text-xs text-slate-400">Pre-Built Strategic Playbooks</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-emerald-400">{formatCurrency(summary?.totalRevenueAtRisk || 0)}</div>
                <div className="text-xs text-slate-400">Revenue Protected This Quarter</div>
              </div>
            </div>
          </div>
        </div>
