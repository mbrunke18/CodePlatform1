...
const STACK_LAYERS = [
  {
    ...
    readinessItems: ["180 cross-industry Readiness Protocols", "231 pre-mapped situations", "12-minute mobilization vs. 30-day baseline", "3,600× execution head start"],
    highlight: true,
    color: GOLD,
    badge: "PRIMARY LAYER",
  ...
  },
  {
    ...
    readinessRole: "Signal intelligence layer. Continuous monitoring of 39 live feeds scored against 231 detection thresholds, with semantic embedding-based similarity scoring augmenting keyword evaluation. Activation history forms the long-term institutional memory layer.",
    readinessItems: ["39 pre-configured signal sources, every 15 minutes", "231 detection thresholds — keyword + semantic embedding scoring", "Semantic similarity via text-embedding-3-small cosine match", "Institutional memory: activation records, debriefs, outcome data"],
    highlight: false,
    color: TEAL,
    badge: "INTELLIGENCE",
    badgeBg: "rgba(43,138,110,0.1)",
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 3 }}>
            {[
              ...
              },
              {
                ...
                badgeBg: "rgba(43,138,110,0.1)",
                badgeBorder: "rgba(43,138,110,0.3)",
                answer: "The IDEA Framework routes every signal through five stages: IDENTIFY the situation category, DETECT the matching detection threshold, present for executive AUTHORIZATION, EXECUTE the pre-staged protocol, and ADVANCE with a structured debrief. Each stage is pre-built — the orchestration runs in the background continuously, so when a trigger fires, the routing is already complete. Execution takes 12 minutes because the coordination happened before the crisis.",
                highlight: false,
              },
            ].map((item, i) => (
              <div key={i} style={{
